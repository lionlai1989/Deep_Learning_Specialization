...
def EncoderLayer_test(target):
    ...
    assert np.allclose(encoded.numpy(), 
                       ...
                       [ 0.14880152, -0.48318022, -1.1908402 ,  1.5252188 ]], atol=1e1), "Wrong values when training=True"
    ...
    assert np.allclose(encoded.numpy(), [[ 0.5167701 , -0.92981905, -0.9731106 ,  1.3861597 ],
                           ...
                           [ 0.28154755, -0.3661362 , -1.3330412 ,  1.4176297 ]], atol=1e1), "Wrong values when training=False"
    print("\033[92mAll tests passed")
    ...
    assert np.allclose(encoderq_output.numpy(), 
                       [[[-0.6906098 ,  1.0988709 , -1.260586  ,  0.85232526],
                        ...
                        [[-0.4612937 ,  1.0697356 , -1.4127715 ,  0.8043293 ],
                         ...
                         [ 1.2370994 , -1.0687275 , -0.8945037 ,  0.7261319 ]]], atol=1e1), "Wrong values case 1"
    ...
    assert np.allclose(encoderq_output.numpy(), 
                       [[[-0.36764443,  0.98527074, -1.4714274 ,  0.85380095],
                         ...
                         [[ 0.01838917,  1.038109  , -1.6154225 ,  0.55892444],
                           ...
                           [ 0.534565  , -0.70262754, -1.18215   ,  1.3502126 ]]], atol=1e1), "Wrong values case 2"
    ...
    assert np.allclose(encoderq_output.numpy(), 
                       [[[-0.5642399 ,  1.0386591 , -1.3530676 ,  0.87864864],
                         ...
                         [[-0.01885331,  0.8866553 , -1.624897  ,  0.75709504],
                           ...
                           [ 0.71298015, -0.7565592 , -1.1861688 ,  1.2297478 ]]], atol=1e1), "Wrong values case 3"
...
def DecoderLayer_test(target, create_look_ahead_mask):
    ...
    assert np.allclose(attn_w_b1[0, 0, 1], [0.5271505,  0.47284946, 0.], atol=1e0), "Wrong values in attn_w_b1. Check the call to self.mha1"
    assert np.allclose(attn_w_b2[0, 0, 1], [0.32048798, 0.390301, 0.28921106], atol=1e0),  "Wrong values in attn_w_b2. Check the call to self.mha2"
    assert np.allclose(out[0, 0], [-0.22109576, -1.5455486, 0.852692, 0.9139523], atol=1e1), "Wrong values in out"
    

    # Now let's try a example with padding mask
    padding_mask = np.array([[[1, 1, 0]]])
    out, attn_w_b1, attn_w_b2 = decoderLayerq(q, encoderq_output, True, look_ahead_mask, padding_mask)
    assert np.allclose(out[0, 0], [0.14950314, -1.6444231, 1.0268553, 0.4680646], atol=1e1), "Wrong values in out when we mask the last word. Are you passing the padding_mask to the inner functions?"
...
def Decoder_test(target, create_look_ahead_mask, create_padding_mask):
    ...
    assert np.allclose(outd[1, 1], [-0.2715261, -0.5606001, -0.861783, 1.69390933], atol=1e1), "Wrong values in outd"
    ...
    assert np.allclose(att_weights[keys[0]][0, 0, 1], [0.52145624, 0.47854376, 0.], atol=1e1), f"Wrong values in att_weights[{keys[0]}]"
    
    outd, att_weights = decoderk(x, encoderq_output, True, look_ahead_mask, None)
    assert np.allclose(outd[1, 1], [-0.30814743, -0.6213016, -0.77767026, 1.7071193], atol=1e1), "Wrong values in outd when training=True"

    outd, att_weights = decoderk(x, encoderq_output, True, look_ahead_mask, create_padding_mask(x))
    assert np.allclose(outd[1, 1], [-0.0250004, 0.50791883, -1.5877104, 1.1047921], atol=1e1), "Wrong values in outd when training=True and use padding mask"
...
def Transformer_test(target, create_look_ahead_mask, create_padding_mask):
    ...
    assert np.allclose(translation[0, 0, 0:8],
                       [0.017416516, 0.030932948, 0.024302809, 0.01997807,
                        0.014861834, 0.034384135, 0.054789476, 0.032087505], atol=1e0), "Wrong values in translation"
    ...
    assert np.allclose(weights[keys[0]][0, 0, 1], [0.4805548, 0.51944524, 0.0, 0.0, 0.0], atol=1e0), f"Wrong values in weights[{keys[0]}]"
    ...
    assert np.allclose(translation[0, 0, 0:8],
                       [0.01751175, 0.029051155, 0.024785805, 0.020421047, 
                        0.0149451075, 0.033235606, 0.053800166, 0.028556924], atol=1e0), "Wrong values in outd"
